...
function change_data() {
    ...
    $("#quiz-main").append('<div id="question-container">Q.' + page + ' ' + q_data + '</div>')

    for (let i = 0; i < chise_data.length; i++) {
        $("#quiz-main").append('<div id="answer-buttons" class="next-button btn">' + chise_data[i] + '</div>')
    }
...
function anser_data_drow(e) {
    ...
    if (sentakudata == answer) {
        $("#quiz-main").append('<div class="hollow-circle"></div>')
        $("#quiz-main").append('<div id="question-container">正解です！！</div>')
        answerCount++;
    } else {
        $("#quiz-main").append('<div class="cross"></div>')
        $("#quiz-main").append('<div id="question-container">不正解！正解は' + answer + '!!</div>')
    }

    if (page == MaxPage) {
        $("#quiz-main").append('<div id="answer-buttons" class="next-button btn">結果を見る</div>')
        nextPage = CST_END_Page;
    } else {
        $("#quiz-main").append('<div id="answer-buttons" class="next-button btn">次の問題へ</div>')
...
function End_drow() {
    $("#quiz-main").empty();
    $("#quiz-main").append('<h1 id="question-container">正解数' + answerCount + '/' + MaxPage + '</h1>');
    $("#quiz-main").append('<div id="answer-buttons" class="next-button btn">タイトルへ</div>');
